...
#### 1.2 耗材分类数据访问层（预计1小时）
- [ ] 创建耗材分类Mapper接口 `MaterialCategoryMapper`
  - 继承BaseMapper获得基础CRUD方法 ✅
  - 自定义查询方法：根据分类编码查询、根据父分类ID查询子分类、分页查询 ✅
  - 查询分类树形结构方法（递归查询） ✅
  - 查询分类及其所有子分类方法 ✅
  - 配置MyBatis-Plus注解 ✅
- [ ] 创建耗材分类Repository接口 `MaterialCategoryRepository`
...
- [ ] 创建耗材分类Service接口 `IMaterialCategoryService`
  - 定义分类创建、更新、删除、查询等业务方法接口 ✅
  - 定义分类树形结构查询方法接口 ✅
  - 定义分类层级管理方法接口 ✅
- [ ] 创建耗材分类Service实现类 `MaterialCategoryServiceImpl`
...
#### 1.4 耗材分类控制层（预计0.5小时）
- [ ] 创建耗材分类Controller `MaterialCategoryController`
  - 分类创建接口 `/api/category` ✅
...
#### 4.1 后端接口测试
- [ ] 创建测试类MaterialCategoryControllerTest
- [ ] 创建测试环境配置application-test.yml
- [ ] 配置H2内存数据库
- [ ] 编写测试用例
  - [ ] 分类创建接口测试（正常创建、编码重复、父分类不存在）
  - [ ] 分类更新接口测试（正常更新、分类不存在、编码重复）
  - [ ] 分类删除接口测试（正常删除、有子分类、有关联耗材）
  - [ ] 分类树形结构查询测试（树形结构正确、层级正确）
  - [ ] 分类移动测试（正常移动、移动到子分类、移动到自己）
  - [ ] 批量删除测试（正常删除、部分失败）
  - [ ] 边界条件测试（层级超过10级、分类名称过长）
  - [ ] 性能测试（大量分类数据查询）
  - [ ] 异常处理测试（数据库异常、并发操作）
- [ ] 执行测试并验证结果
- [ ] 生成测试覆盖率报告
